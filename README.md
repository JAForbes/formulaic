...
A very, _very_ simple excel-_like_ formula engine that uses "clever" regex search replace to create a (hopefully) sandboxed JS script that is then eval'd.
...
This library tries (desperately) to sanitize the input using some clever tricks.

First it uses an allow list that makes it (hopefully) not possible to do clever exploits in JS.  For example no square brackets, so no dynamic keys.  And no use of `.` except in floating point numbers, so no property access.
...
So `A1 + sum(A:C)` becomes `fn.addr("A1") + fn.sum("A:C")`.  The thinking is, you can provide any functions you like as the `fn` context and there is (again hopefully) no way to escape the sandbox and dynamic reach the global object via prototype jumping.
